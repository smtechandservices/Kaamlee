...
export default function NewEventPage() {
  ...
  // Debounced live map preview as the ambassador pastes a Google Maps link —
  // lets them confirm the pin lands in the right place before submitting.
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    if (form.location_map_url.trim()) formData.append('location_map_url', form.location_map_url.trim());
    ...
    if (form.goodies.trim()) formData.append('goodies', form.goodies.trim());
    if (form.collaborators.trim()) formData.append('collaborators', form.collaborators.trim());
    if (file) formData.append('banner_image', file);

    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/events/requests/`, {
        method: 'POST',
  ...
  if (status === 'success') {
    return (
      <div className="mx-auto w-full">
        <span className="mx-auto block w-fit text-[22px] font-bold text-[#16A34A] -rotate-2" style={{ fontFamily: 'var(--font-caveat)' }}>nice work,</span>
        <div className="relative mt-3 overflow-hidden rounded-[28px] border border-black/[0.08] bg-white p-10 text-center shadow-[0_1px_2px_rgba(16,18,26,.05),0_20px_48px_-20px_rgba(16,18,26,.14)]">
          ...
          <h1 className="relative mt-6 text-[26px] font-semibold tracking-[-0.03em]" style={{ fontFamily: 'var(--font-outfit)' }}>Request submitted</h1>
          <p className="relative mx-auto mt-3 max-w-sm text-[14.5px] leading-relaxed text-black/55">
            {form.title ? <>We&apos;ve got <span className="font-semibold text-black/75">&ldquo;{form.title}&rdquo;</span>, an</> : 'An'} &nbsp;admin will review it, and it&apos;ll show up as approved (or rejected) on your Events page.
          </p>
          <Link
            href="/events"
  ...
  return (
    <div>
      ...
      <h1 className="mt-4 font-[var(--font-outfit)] text-[30px] font-semibold tracking-tighter">Submit an event</h1>
      <p className="mt-2 max-w-xl text-[14.5px] leading-relaxed text-[#6B7280]">
        Tell us what you&apos;re planning — a resume clinic, alumni AMA, mock interview night, whatever fits your campus. An admin reviews it before it goes live.
      </p>
        ...
        <div className="flex flex-col gap-5">
          ...
          <Field id="banner_image" label="Banner image (optional)" error={errors.banner_image}>
            ...
            >
              ...
              <span className="truncate">{file ? `${file.name} (${(file.size / (1024 * 1024)).toFixed(1)} MB)` : 'Upload a banner image for the event'}</span>
            </label>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <button
              ...
              style={{ fontFamily: 'var(--font-outfit)' }}
            >
              {status === 'submitting' ? (<><Loader2 size={17} className="animate-spin" /> Submitting…</>) : 'Submit for review'}
            </button>
          </div>
        </div>
      </div>
