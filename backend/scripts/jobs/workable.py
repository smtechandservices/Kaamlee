"""Pull job postings from a Workable job board and save them into the Job model.

Workable exposes a public, unauthenticated read API per company board via the
same widget its embeddable careers page uses:

    https://apply.workable.com/api/v1/widget/accounts/{client_name}

`client_name` is the slug in the board's public URL, e.g. the "pavago" in
https://apply.workable.com/pavago. Like Lever, Workable's API is
case-sensitive about this slug (PAVAGO/Pavago both 404; only /pavago
...
posting in a single response (no pagination, no `limit` param needed).

A single job posted to more than one country comes back as multiple entries
in `jobs` that share the same `shortcode` but differ by country/city/state —
Workable's widget has already exploded it into one row per location, unlike
Lever/Greenhouse where one entry carries an array of locations. id_from_site
is keyed on shortcode + location so those rows don't collide.

This endpoint has no per-job description or salary field (only a company-wide
blurb, which isn't a per-job description) — Job.description and Job.salary
are left unset for postings pulled from here.

Usage:
    python scripts/jobs/workable.py pavago
    python scripts/jobs/workable.py pavago figma
"""
import argparse
import os
import sys
...
def fetch_board(client_name):
    ...
    response = requests.get(
        f"https://apply.workable.com/api/v1/widget/accounts/{client_name}",
...
# ------------------------------------------------------------------
# Field mapping helpers — one posting (raw Workable JSON) -> Job model fields
# ------------------------------------------------------------------
...
def sync_board(client_name, company_name=None, stop_event=None):
    ...
    them (Workable gives no coordinates of its own — just city/state/country
    strings per posting, same as the other ATSes). Any job that still has no
    coordinates afterwards is dropped, since a job with no coordinates can't
    be plotted on the map.

    id_from_site is namespaced as "workable:{client}:{shortcode}:{location}"
    — the location suffix matters here because one shortcode can appear once
    per country it's posted to (see module docstring) — so re-running this
    for the same board updates existing rows instead of duplicating them,
    and IDs can't collide with jobs pulled in from a different ATS.
    client_name is lowercased once into client_key and used everywhere a
    request or URL is built, not just for id_from_site, since Workable's API
    is case-sensitive about the board slug.
    ...
    board_url = f"https://apply.workable.com/{client_key}/"

    yield f"Fetching https://apply.workable.com/api/v1/widget/accounts/{client_key} ..."
    ...
    for i, job in enumerate(jobs, start=1):
        ...
        obj = Job(
            ...
            job_url=job.get('url') or job.get('shortlink') or board_url,
            description=None,
            site=board_url,
    ...
    rate_limited = False
    if not stopped:
        # Workable gives no coordinates directly — geocode every job just
    ...
    if stopped:
        ...
        removed = Job.objects.filter(
            id_from_site__startswith=f"workable:{client_key}:", latitude__isnull=True,
        ).delete()[0]
        if removed:
...
def run_many(client_names, on_log=None, on_result=None, stop_events=None):
    ...
    def worker(entry):
        ...
        try:
            ...
            result = {'board': client_name, 'ok': False, 'error': f"Workable board not found or unavailable (HTTP {status})"}
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'board_names', nargs='+',
        help='One or more Workable board slugs, e.g. "pavago" from apply.workable.com/pavago',
    )
