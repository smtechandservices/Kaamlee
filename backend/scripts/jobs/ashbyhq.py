...
def sync_board(board_name, company_name=None, stop_event=None):
    ...
    for i, job in enumerate(jobs, start=1):
        ...
        # returns it in the feed for a while — don't save those, and don't
        # count it towards fetched_ids either so the stale-postings cleanup
        # below removes it if it was saved on an earlier run.
    ...
    # at all (closed, removed, or unlisted per the isListed check above) are
    # gone for good, not just uncoordinated — same stopped/empty-fetch guards
    # as above, since either case means we don't actually know the board's
    ...
    if stopped:
        ...
        removed_stale = remove_stale_jobs(f"ashby:{board_key}:", fetched_ids)
    ...
    removed_old = remove_old_jobs(f"ashby:{board_key}:")
    removed += removed_old
    if removed_old:
