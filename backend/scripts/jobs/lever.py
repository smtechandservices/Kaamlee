"""Pull job postings from a Lever job board and save them into the Job model.

Lever exposes a public, unauthenticated read API per company board:

    https://api.lever.co/v0/postings/{client_name}?mode=json&limit=100

`client_name` is the slug in the board's public URL, e.g. the "palantir" in
https://jobs.lever.co/palantir. Lever's API is case-sensitive about this
slug, unlike Greenhouse/Ashby (jobs.lever.co/Palantir 404s; only /palantir
works) — sync_board() lowercases it automatically, so it's safe to pass a
company's normal display name here too. The endpoint returns every listed
posting in a single response given a large enough `limit` (no pagination
needed — see fetch_board's docstring for why `offset` isn't used here).

Usage:
    python scripts/jobs/lever.py palantir
    python scripts/jobs/lever.py palantir figma
"""
import argparse
import os
import sys
...
# Lever's `offset` query param isn't a working cursor on this endpoint in
# practice — passing the previous page's last id (or a numeric skip count)
# both just return page 1 again, which would infinite-loop a naive
# paginator. A single request with a generously large `limit` reliably
# returns everything instead (verified against a 284-posting board with no
# truncation up to limit=5000), so that's what we do here — same
# one-call-gets-everything shape as Greenhouse/Ashby.
FETCH_LIMIT = 10000
...
def fetch_board(client_name):
    """Fetch every listed posting for a board in one call."""
    response = requests.get(
        f"https://api.lever.co/v0/postings/{client_name}",
        params={'mode': 'json', 'limit': FETCH_LIMIT},
...
# ------------------------------------------------------------------
# Field mapping helpers — one posting (raw Lever JSON) -> Job model fields
# ------------------------------------------------------------------
def _is_remote(job, location_name):
    if job.get('workplaceType'):
        return job['workplaceType'] == 'remote'
...
def _date_posted(job):
    ...
    if not created_at:
        return None
    try:
        return datetime.fromtimestamp(created_at / 1000, tz=dt_timezone.utc).date()
...
def _salary(job):
    ...
    lo, hi, currency, interval = (
        salary_range.get('min'), salary_range.get('max'), salary_range.get('currency'), salary_range.get('interval'),
    )
    if lo or hi:
    ...
    description = job.get('salaryDescription')
...
def sync_board(client_name, company_name=None, stop_event=None):
    ...
    them (Lever gives no coordinates of its own — just a free-text location
    string per posting, same as Greenhouse). Any job that still has no
    coordinates afterwards — its own location couldn't be geocoded and no
    company sibling had one to borrow — is dropped, since a job with no
    coordinates can't be plotted on the map.

    id_from_site is namespaced as "lever:{client}:{job id}" so re-running
    this for the same board updates existing rows instead of duplicating
    them, and so IDs can't collide with jobs pulled in from a different
    ATS. Unlike Greenhouse/Ashby, Lever's API is case-SENSITIVE about the
    board slug (jobs.lever.co/Palantir 404s; only /palantir works) — so
    client_name is lowercased once into client_key and used everywhere a
    request or URL is built, not just for id_from_site. Otherwise typing
    "Palantir" (matching Company.name, as the admin dashboard naturally
    does) would 404 instead of silently working like it does for the other
    two ATSes.
    ...
    board_url = f"https://jobs.lever.co/{client_key}"

    yield f"Fetching https://api.lever.co/v0/postings/{client_key} ..."
    jobs = fetch_board(client_key)
    ...
    rate_limited = False
    if not stopped:
        # Lever gives no coordinates directly — geocode every job just
    ...
    if stopped:
        ...
        removed = Job.objects.filter(
            id_from_site__startswith=f"lever:{client_key}:", latitude__isnull=True,
        ).delete()[0]
        if removed:
    ...
    if stopped:
        ...
        removed_stale = remove_stale_jobs(f"lever:{client_key}:", fetched_ids)
    ...
    removed_old = remove_old_jobs(f"lever:{client_key}:")
    removed += removed_old
    if removed_old:
...
def run_many(client_names, on_log=None, on_result=None, stop_events=None):
    ...
    def worker(entry):
        ...
        try:
            ...
            result = {'board': client_name, 'ok': False, 'error': f"Lever board not found or unavailable (HTTP {status})"}
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'board_names', nargs='+',
        help='One or more Lever board slugs, e.g. "palantir" from jobs.lever.co/palantir',
    )
