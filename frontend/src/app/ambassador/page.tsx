...
const TESTIMONIALS = [
  { big: true, quote: 'I joined for the stipend and left with a PPO. Running the resume clinic taught me more about hiring than any course I took.', initials: 'AK', avatarBg: '#16A34A', avatarFg: '#fff', name: 'Aditi K.', meta: 'VIT Vellore · now Product intern at Kaamlee' },
  ...
  return (
    <div className={`${caveat.variable} bg-[#FAF9F6] text-[#0A0A0A] min-w-[320px] overflow-x-clip`} style={{ fontFamily: 'var(--font-jakarta), ui-sans-serif, system-ui, sans-serif' }}>
      ...
      <header className="sticky top-0 z-50 bg-[#FAF9F6]/80 backdrop-blur-md border-b border-[#E7E5E0]">
        <div className="max-w-6xl mx-auto px-6 md:px-10 h-16 flex items-center gap-4 md:gap-7">
      ...
      {/* Hero */}
      <section id="top" className="relative px-6 md:px-10 pt-16 pb-20 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_560px] gap-10 items-start relative z-10">
      ...
      {/* Community */}
      <section id="community" className="relative px-6 md:px-10 py-24 md:py-28 overflow-hidden">
      ...
      {/* What Do You Get */}
      <section id="get" className="px-6 md:px-10 py-24 md:py-28">
        <div className="max-w-6xl mx-auto">
          ...
          {/* <Reveal className="mt-10 grid grid-cols-2 gap-3">
            <div className="grid grid-rows-[160px_200px] gap-3">
              <ImageSlot label="Ambassadors laughing" gradient="linear-gradient(135deg,#34D399,#16A34A)" />
              <ImageSlot label="Campus walk" gradient="linear-gradient(135deg,#4F46E5,#0A0A0A)" />
            </div>
            <div className="grid grid-rows-[240px_120px] gap-3">
              <ImageSlot label="Portrait, ambassador outdoors" gradient="linear-gradient(135deg,#FBBF24,#FF4D2E)" />
              <ImageSlot label="Event stage wide" gradient="linear-gradient(135deg,#0A0A0A,#374151)" />
            </div>
          </Reveal> */}
      ...
      {/* What Will You Do */}
      <section id="do" className="px-6 md:px-10 py-4 pb-24 md:pb-28">
        <div className="max-w-6xl mx-auto">
          <Reveal className="relative">
            <div className="hidden md:block absolute -left-8 -top-8 w-36 h-36 bg-[#4F46E5] animate-[spin_50s_linear_infinite]" style={{ clipPath: 'polygon(50% 0%,58% 32%,79% 8%,71% 38%,97% 27%,80% 50%,97% 73%,71% 62%,79% 92%,58% 68%,50% 100%,42% 68%,21% 92%,29% 62%,3% 73%,20% 50%,3% 27%,29% 38%,21% 8%,42% 32%)' }} />
            <h2 className="relative font-[var(--font-outfit)] text-5xl md:text-7xl lg:text-[80px] leading-[0.92] font-medium tracking-tighter">
              What Will
              <br />
              <span className="md:ml-32 inline-block">You Do?</span>
            </h2>
            <p className="mt-6 md:ml-32 font-[var(--font-outfit)] text-xl md:text-2xl leading-snug text-[#374151] max-w-md">
              Six to eight hours a month. That&apos;s the whole ask.
            </p>
          </Reveal>

          <Reveal className="mt-14 max-w-3xl mx-auto flex flex-col">
            {TASKS.map((t, i) => (
              <div
                key={t.em}
                className={`relative text-white px-8 sm:px-14 py-8 sm:py-10 ${i > 0 ? '-mt-2' : ''}`}
                style={{
                  background: 'linear-gradient(#1C1C1C, #000000)',
                  clipPath: 'polygon(2% 0, 98% 0, 100% 100%, 0 100%)',
                }}
              >
                <span className="absolute left-6 sm:left-9 top-4 text-sm font-bold text-[#737373]">{'→'}</span>
          ...
          <Reveal className="mt-10 flex items-center justify-center gap-3 text-sm text-[#6B7280] font-semibold">
      ...
      {/* Who Can Apply */}
      <section id="who" className="px-6 md:px-10 py-4 pb-28 md:pb-32">
        <div className="max-w-6xl mx-auto">
          <Reveal className="text-center">
      ...
      {/* Stories */}
      <section id="stories" className="px-6 md:px-10 pb-24 md:pb-28">
        <div className="max-w-6xl mx-auto">
          ...
          <Reveal className="mt-10 grid grid-cols-1 lg:grid-cols-[1.25fr_1fr_1fr] gap-4">
            {TESTIMONIALS.map((t) => (
              <div
                key={t.name}
                className={`bg-white border border-[#E7E5E0] rounded-[22px] flex flex-col gap-5 ${t.big ? 'p-8' : 'p-7'}`}
              >
                <p className={t.big ? 'font-[var(--font-outfit)] text-2xl leading-snug font-medium tracking-tight' : 'text-lg leading-relaxed text-[#374151]'}>
                  &ldquo;{t.quote}&rdquo;
                </p>
      ...
      {/* CTA */}
      <section id="apply" className="px-6 md:px-10 pb-24 md:pb-28">
        <Reveal className="max-w-6xl mx-auto relative overflow-hidden bg-[#4ADE80] text-[#04231A] rounded-[30px] px-8 md:px-12 py-20 md:py-24 text-center">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
              <a
                href="mailto:campus@kaamlee.com"
      ...
      <footer className="border-t border-[#E7E5E0]">
        <div className="max-w-6xl mx-auto px-6 md:px-10 pt-12 pb-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.7fr_1fr_1fr_1fr] gap-10">
          <div>
            <div className="flex items-center gap-2.5">
              <span className="w-7 h-7 rounded-lg bg-[#16A34A] grid place-items-center text-white font-extrabold text-sm font-[var(--font-outfit)]">K</span>
              <span className="text-lg font-bold tracking-tight font-[var(--font-outfit)]">kaamlee</span>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/" className="text-sm text-[#374151] font-medium hover:text-[#0A0A0A]">For candidates</Link>
            <Link href="/pricing" className="text-sm text-[#374151] font-medium hover:text-[#0A0A0A]">For employers</Link>
            <Link href="/privacy" className="text-sm text-[#374151] font-medium hover:text-[#0A0A0A]">Privacy</Link>
          </div>
        </div>
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-5 border-t border-[#E7E5E0] flex flex-col sm:flex-row justify-between gap-2 text-sm text-[#78716C]">
